...
public class Schedule {
    ...
    /**
     * Find start time of all one hour breaks less than the end time
     */

    public Set<String> getStartTimesOfOneHourBreaks(String dayOfWeek) {
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        Set<String> startTimes = new HashSet<String>();
        if (sections.size() == 1) {
            Section section = sections.first();
    ...
    public Building whereAmITwoPointOh(String dayOfWeek, int timeOfDay) {
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        // Find which section ended just before timeOfDay
        Section lastSection = null;
        for (Section section : sections) {
            String s = section.getCourseTime().getEndTime();
            int sectionMinutesIntoDay = calculateMinutesIntoDay(s);
            int timeToMeetIntoMinutes = timeOfDay * 60;
            if (sectionMinutesIntoDay <= timeToMeetIntoMinutes) {
                lastSection = section;
            }
        }
        if (lastSection != null)
